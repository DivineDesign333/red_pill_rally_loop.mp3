...
install_cuda() {
    ...
    if command -v nvidia-persistenced > /dev/null 2>&1; then
        ...
        for MODULE in $MODULES; do
            MODULES_PATH="/etc/modules-load.d/nvidia.conf"
            if ! grep -xq "$MODULE" $MODULES_PATH; then
                echo "$MODULE" | $SUDO tee -a $MODULES_PATH > /dev/null
            fi
        done
    fi
}
...
install_amd() {
    # AMD GPU support for ROCm
    # ref: https://rocm.docs.amd.com/projects/install-on-linux/en/latest/how-to/native-install/index.html
    if ! available lspci; then
        warn "Unable to verify AMD ROCm support. Install lspci to automatically detect and install ROCm."
        return
    fi

    if ! check_gpu lspci amdgpu; then
        warn "AMD GPU not detected. Skipping ROCm installation."
        return
    fi

    if [ ! -f "/etc/os-release" ]; then
        warn "Unknown distribution. Skipping ROCm installation."
        return
    fi
    ...
    info "Installing AMD ROCm..."

    # We only support Ubuntu for now
    if [ "$OS_NAME" = "ubuntu" ]; then
        info "Installing ROCm for Ubuntu..."
        curl -fsSL https://repo.radeon.com/rocm/rocm.gpg.key | gpg --dearmor | $SUDO tee /usr/share/keyrings/rocm.gpg > /dev/null
        echo "deb [arch=amd64 signed-by=/usr/share/keyrings/rocm.gpg] https://repo.radeon.com/rocm/apt/${OS_VERSION} ubuntu main" | $SUDO tee /etc/apt/sources.list.d/rocm.list
        $SUDO apt update
        $SUDO apt install -y rocm-hip-sdk
    else
        warn "ROCm installation is only supported on Ubuntu. Skipping..."
    fi
}
...
install_docker() {
    ...
    HAS_AMD=${HAS_AMD:-false}
    HAS_INTEL=${HAS_INTEL:-false}
    HAS_VULKAN=${HAS_VULKAN:-false}
    ...
    if [ "$WORKER" = true ]; then
        envs="$envs -e LOCALAI_WORKER=true"
    fi

    if [ "$FEDERATED" = true ]; then
        envs="$envs -e LOCALAI_FEDERATED=true"
    fi

    if [ "$FEDERATED_SERVER" = true ]; then
        envs="$envs -e LOCALAI_FEDERATED_SERVER=true"
    fi
    ...
    # Check if we have already the container running
    if $SUDO docker ps --format '{{.Names}}' | grep -q local-ai; then
        info "LocalAI container already exists. Stopping and removing..."
        $SUDO docker stop local-ai
        $SUDO docker rm local-ai
    fi
    ...
    if [ "$HAS_CUDA" = true ]; then
        ...
        info "Starting LocalAI Docker container with NVIDIA GPU support..."
        ...
        # AIO
        if [ "$USE_AIO" = true ]; then
            IMAGE_TAG=${LOCALAI_VERSION}-aio-vulkan
        fi

        info "Starting LocalAI Docker container with Vulkan GPU support..."
        $SUDO docker run -v local-ai-data:/models \
            --device /dev/dri \
            --restart=always \
            -e API_KEY=$API_KEY \
